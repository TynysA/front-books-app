    ...
    <div tw='px-4 flex-grow flex flex-col justify-between gap-[62px]'>
      <div tw=''>
        <div tw='flex flex-col gap-[16px] mb-[24px]'>
          <div tw='text-[#0F0F0F] text-[28px] font-bold'>Cогласие на сбор  и обработку данных</div>
        ...
        {newUsers.map((item: any, idx) => (
          <div key={idx} tw='flex flex-col bg-[#FFFFFF] mb-[12px] p-[16px] gap-[16px] text-black rounded-[16px]'>
            <div tw='flex gap-[12px] items-center border-b-[1px] border-[#EAECED] pb-[16px]'>
              ...
              <div>
                <p tw='text-[#0F0F0F] text-[16px] font-semibold'>{item.fullName}</p>
                <span tw='text-[#8E8E93] text-[13px]'>{item.iin}</span>
              </div>
            </div>
            <div tw='text-[#3A3A3C] text-[16px]'>Ожидает подтверждение</div>
      ...
      {isAgree ? (
        ...
        <Timer variant={'re-register'} initialSeconds={5} resendOtp={onResend} handele={handeleSendSmS}></Timer>
      )}
    </div>
  );
};
